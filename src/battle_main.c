...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_PSYDUCK] =         { _("PSYDUCK"), 1 },
};
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            if (partyData[monIndex].ability != ABILITY_NONE)
            {
                ...
                {
                    abilityNum = 0;
                    customAbility = partyData[monIndex].ability;
                }
            }
            ...
            if (partyData[monIndex].sequenceMoves)
            {
                u32 data = partyData[monIndex].sequenceMoves;
                SetMonData(&party[i], MON_DATA_SEQUENCE_MOVES_FLAG, &data);
            }
...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, enum Ability ability, enum ItemHoldEffect holdEffect)
{
    ...
    if (holdEffect == HOLD_EFFECT_MACHO_BRACE || holdEffect == HOLD_EFFECT_POWER_ITEM)
        ...
        speed = (speed * 150) / 100;
    else if (holdEffect == HOLD_EFFECT_COFFEE && GetActiveGimmick(battler) != GIMMICK_DYNAMAX)
        speed = (speed * 125) / 100;
