...
{
    { COMPOUND_STRING("Move Reminder"),      DebugAction_ExecuteScript, TwoIsland_House_EventScript_ChooseMonToTutor },
...
{
    ZeroPlayerPartyMons();
    CreateNPCTrainerPartyFromTrainer(gPlayerParty, &sDebugTrainers[DIFFICULTY_NORMAL][DEBUG_TRAINER_PLAYER], TRUE, BATTLE_TYPE_TRAINER, 0xffff);
    ScriptContext_Enable();
    Debug_DestroyMenu_Full(taskId);
}
...
static void DebugAction_Party_BattleSingle(u8 taskId)
{
    ...
    ZeroEnemyPartyMons();
    CreateNPCTrainerPartyFromTrainer(gPlayerParty, &sDebugTrainers[DIFFICULTY_NORMAL][DEBUG_TRAINER_PLAYER], TRUE, BATTLE_TYPE_TRAINER, 0xffff);
    CreateNPCTrainerPartyFromTrainer(gEnemyParty, GetDebugAiTrainer(), FALSE, BATTLE_TYPE_TRAINER, 0xffff);
